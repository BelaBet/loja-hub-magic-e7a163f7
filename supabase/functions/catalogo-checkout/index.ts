// Edge function PÚBLICA (sem autenticação de funcionário): cria o pedido de
...
type CardData = { number: string; holder_name: string; exp_month: number; exp_year: number; cvv: string; installments?: number };
...
function normalizeAddress(address: AddressData | undefined) {
  ...
  if (!normalized.street || !normalized.number || !normalized.zip_code || !normalized.neighborhood || !normalized.city || !normalized.state) return null;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    catch (e) { if (e instanceof PlatformRecipientError) return json({ error: "Este catálogo ainda não está pronto para receber pagamentos. Tente novamente mais tarde." }, 503); throw e; }
    ...
    if (itensErr) {
      ...
      const semEstoque = itensErr.message?.toLowerCase().includes("estoque");
