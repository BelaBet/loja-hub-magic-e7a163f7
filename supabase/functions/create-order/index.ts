...
function calculateSplit(baseAmount: number, installments: number, passToCustomer: boolean) {
  ...
  const installmentSurcharge = passToCustomer && safeInstallments > 1 ? Math.round(baseAmount * installmentRate) : 0;
...
function validateCard(card: CardData | undefined, installments: number) {
  ...
  const now = new Date();
  if (!Number.isInteger(card.exp_year) || card.exp_year < now.getUTCFullYear() || (card.exp_year === now.getUTCFullYear() && card.exp_month < now.getUTCMonth() + 1)) return "Cartão expirado";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { payment_method, amount, customer, items, card, venda_id, pass_surcharge_to_customer = true } = body;
    ...
    if (!lojaId) {
      ...
      const { data: lu } = await admin.from("loja_usuarios").select("loja_id, created_at").eq("user_id", userId).order("created_at", { ascending: true }).limit(1).maybeSingle();
      lojaId = lu?.loja_id ?? null;
    }

    let seller_recipient_id: string | null = null;
    if (lojaId) {
      const { data: loja } = await admin.from("lojas").select("pagarme_recipient_id").eq("id", lojaId).maybeSingle();
    ...
    if (venda_id) {
      ...
      const { data: venda, error: vendaError } = await admin.from("vendas").select("id").eq("id", venda_id).eq("loja_id", lojaId).maybeSingle();
